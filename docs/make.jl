...
function build_battmo_docs(
    ...
    if clean
        for (ex, pth) in tutorials
            ...
            if isfile(delpath)
                ...
                println(
                    "Did not find generated example \"$ex\", skipping removal:\n\t$delpath"
                )
            end
        end
    ...
    if isnothing(build_format)
        build_format = DocumenterVitepress.MarkdownVitepress(;
            repo="github.com/BattMoTeam/BattMo.jl", devbranch="main", devurl="dev"
        )
    end
    ...
    makedocs(;
        ...
        pages=[
            ...
            "Examples" => ["Advanced examples" => examples_markdown],
            "API Documentation" =>
            ...
            "References" => ["Bibliography" => "extras/refs.md"],
        ],
    )
    if build_notebooks
