...
function update_json_input(;
    ...
    if y_name == "openCircuitPotential"
        if !(x_name in ["SOC", "c", "T", "cmax"])
            error(
                "ValueError: The x_name: '$x_name'is not recognized by the 'update_json_input' function. Please enter 'SOC', 'c', 'T' or 'cmax'.",
            )
        end
        if haskey(json_data, component_name)
            if haskey(json_data[component_name], "Coating")
                if haskey(json_data[component_name]["Coating"], "ActiveMaterial")
                    if haskey(
                        json_data[component_name]["Coating"]["ActiveMaterial"], "Interface"
                    )
                        if haskey(
                            json_data[component_name]["Coating"]["ActiveMaterial"]["Interface"],
                            "openCircuitPotential",
                            ...
                            if haskey(
                                json_data[component_name]["Coating"]["ActiveMaterial"]["Interface"]["openCircuitPotential"],
                                "function",
                            )
                                json_data[component_name]["Coating"]["ActiveMaterial"]["Interface"]["openCircuitPotential"]["function"] =
                                    expression
                                json_data[component_name]["Coating"]["ActiveMaterial"]["Interface"]["openCircuitPotential"]["argumentlist"] = [
                                    x_name
                                ]
                            elseif haskey(
                                json_data[component_name]["Coating"]["ActiveMaterial"]["Interface"]["openCircuitPotential"],
    ...
    elseif y_name == "ionicConductivity" || y_name == "diffusionCoefficient"
        if !(x_name in ["c", "T"])
            error(
                "ValueError: The x_name: '$x_name'is not recognized by the 'update_json_input' function. Please enter 'c' or 'T'.",
            )
        end
        ...
        error(
            "ValueError: The y_name: '$y_name'is not recognized by the 'update_json_input' function. Please enter 'openCircuitPotential', 'ionicConductivity' or 'diffusionCoefficient'.",
        )
    end
