...
function show(io::IO, report::ValidationReport)
    return if isempty(report.issues)
...
function _validate(x, schema, path::String, issues::Vector{SingleIssue})
    ...
    return _validate_entry(x, schema, path, issues)
end
...
# 9.2.1.1
function _validate(x, schema, ::Val{:allOf}, val::AbstractVector, path::String, issues::Vector{SingleIssue})
    for v in val
...
function _validate(x, schema, ::Val{:anyOf}, val::AbstractVector, path::String, issues::Vector{SingleIssue})
    ...
    return push!(issues, SingleIssue(x, path, "anyOf", val))
end

# 9.2.1.3
function _validate(x, schema, ::Val{:oneOf}, val::AbstractVector, path::String, issues::Vector{SingleIssue})
    found_match = false
    temp_issues = Vector{SingleIssue}()
...
function _validate(x, schema, ::Val{:not}, val, path::String, issues::Vector{SingleIssue})
    ...
    return if isempty(local_issues)
...
function _validate(x, schema, ::Val{:if}, val, path::String, issues::Vector{SingleIssue})
    return if haskey(schema, "then") || haskey(schema, "else")
...
# 9.3.1.1
function _validate(x::AbstractVector, schema, ::Val{:items}, val::AbstractDict, path::String, issues::Vector{SingleIssue})
    ...
    return _additional_items(x, schema, items, additionalItems, path, issues)
end

function _validate(x::AbstractVector, schema, ::Val{:items}, val::AbstractVector, path::String, issues::Vector{SingleIssue})
    items = fill(false, length(x))
    for (i, xi) in enumerate(x)
        if i > length(val)
            break
...
function _validate(x::AbstractVector, schema, ::Val{:items}, val::Bool, path::String, issues::Vector{SingleIssue})
    return if !val && length(x) > 0
...
_additional_items(x, schema, items, val::Nothing, path, issues::Vector{SingleIssue}) = nothing

# 9.3.1.2
function _validate(x::AbstractVector, schema, ::Val{:additionalItems}, val, path::String, issues::Vector{SingleIssue})
...
function _validate(
    ...
    return push!(issues, SingleIssue(x, path, "contains", val))
end
...
# 6.1.1
function _validate(x, schema, ::Val{:type}, val::String, path::String, issues::Vector{SingleIssue})
...
function _validate(x, schema, ::Val{:type}, val::AbstractVector, path::String, issues::Vector{SingleIssue})
...
function _dependencies(x::AbstractDict, path::String, val::Array, issues::Vector{SingleIssue})
