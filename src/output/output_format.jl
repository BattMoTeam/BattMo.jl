...
function compute_voltage_breakdown(output::SimulationOutput)
	...
	for key in required
		if !haskey(states, key)
			error("Cannot compute voltage breakdown. Missing output state \"$key\".")
		end
	end
	...
	phi_n = states["NegativeElectrodeActiveMaterialPotential"]
	phi_p = states["PositiveElectrodeActiveMaterialPotential"]
	phi_e = states["ElectrolytePotential"]
	ocp_n = states["NegativeElectrodeActiveMaterialOpenCircuitPotential"]
	ocp_p = states["PositiveElectrodeActiveMaterialOpenCircuitPotential"]
	c_n_surf = get(states, "NegativeElectrodeActiveMaterialSurfaceConcentration", nothing)
	c_p_surf = get(states, "PositiveElectrodeActiveMaterialSurfaceConcentration", nothing)
	R0_n_field = get(states, "NegativeElectrodeActiveMaterialReactionRateConstant", nothing)
	R0_p_field = get(states, "PositiveElectrodeActiveMaterialReactionRateConstant", nothing)
	c_e = get(states, "ElectrolyteConcentration", nothing)
	c_n_part = get(states, "NegativeElectrodeActiveMaterialParticleConcentration", nothing)
	c_p_part = get(states, "PositiveElectrodeActiveMaterialParticleConcentration", nothing)
	T_n = get(states, "NegativeElectrodeActiveMaterialTemperature", nothing)
	T_p = get(states, "PositiveElectrodeActiveMaterialTemperature", nothing)
	...
	has_sei = haskey(states, "SEIVoltageDrop")
	sei = has_sei ? states["SEIVoltageDrop"] : nothing
	...
	use_net_reactive_kinetics = !isnothing(c_e) &&
		!isnothing(c_n_surf) &&
		!isnothing(c_p_surf) &&
