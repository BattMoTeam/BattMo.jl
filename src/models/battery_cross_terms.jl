function Jutul.cross_term_entities(ct::TPFAInterfaceFluxCT, eq::Jutul.JutulEquation, model)
    ct.target_cells
end
function Jutul.cross_term_entities_source(
    ct::TPFAInterfaceFluxCT, eq::Jutul.JutulEquation, model
)
    ct.source_cells
end
...
function Jutul.cross_term_entities(ct::AccumulatorInterfaceFluxCT, eq::JutulEquation, model)
    [ct.target_cell]
end
...
function reaction_rate_coefficient(R0, c_e, c_a, activematerial)
    F = FARADAY_CONSTANT
...
function reaction_rate(eta, c_a, R0, T, c_e, activematerial, electrolyte)
    F = FARADAY_CONSTANT
...
function Jutul.cross_term_entities(
    ct::ButlerVolmerActmatToElyteCT, eq::JutulEquation, model
)
    ct.target_cells
end
function Jutul.cross_term_entities_source(
...
function Jutul.update_cross_term_in_entity!(
    ...
    R = reaction_rate(eta, c_a, R0, T, c_e, activematerial, electrolyte)
...
function Jutul.cross_term_entities(
    ct::ButlerVolmerElyteToActmatCT, eq::JutulEquation, model
)
    ct.target_cells
end
function Jutul.cross_term_entities_source(
...
function source_electric_material(
    vols, T, phi_a, c_a, R0, ocp, phi_e, c_e, activematerial, electrolyte
)
    n = activematerial.params[:n_charge_carriers]
    vsa = activematerial.params[:volumetric_surface_area]

    R = reaction_rate(phi_a, c_a, R0, ocp, T, phi_e, c_e, activematerial, electrolyte)
...
function Jutul.cross_term_entities(
    ct::ButlerVolmerInterfaceFluxCT, eq::JutulEquation, model
)
    ct.target_cells
end
function Jutul.cross_term_entities_source(
...
function Jutul.update_cross_term_in_entity!(
    ...
    eS, eM = source_electric_material(
        vols, T, phi_a, c_a, R, ocp, phi_e, c_e, activematerial, electrolyte
    )
