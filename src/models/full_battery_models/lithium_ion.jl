...
struct LithiumIonBatteryModel <: BatteryModel
    ...
    function LithiumIonBatteryModel(;
        model_settings=get_default_model_settings(LithiumIonBatteryModel)
    )
        ...
        return new{}(name, model_settings, is_valid)
    end
end

function print_required_cell_parameters(::LithiumIonBatteryModel)
    required_cell_parameters = [("CoatingThickness", "m", "Real"), ("OCV", "V", "Function")]
    ...
    for (name, unit, type) in required_cell_parameters
        println(
            "│ " * lpad(name, 16) * " │ " * lpad(unit, 9) * " │ " * lpad(type, 9) * " │"
        )
    end
    println("└──────────────────┴───────────┴───────────┘")
end
