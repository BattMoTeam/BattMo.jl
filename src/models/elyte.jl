...
export Electrolyte, DmuDc, ChemCoef, p1, p2, p3, diffusivity, ElectrolyteModel
...
function Jutul.select_primary_variables!(S, system::Electrolyte, model::SimulationModel)
    S[:Phi] = Phi()
    S[:C] = C()
end

function Jutul.select_parameters!(S, system::Electrolyte, model::SimulationModel)
    S[:Temperature] = Temperature()
    S[:BruggemanCoefficient] = BruggemanCoefficient()
    S[:VolumeFraction] = VolumeFraction()
end

function Jutul.select_equations!(eqs, system::Electrolyte, model::SimulationModel)
...
function Jutul.select_secondary_variables!(S, system::Electrolyte, model::SimulationModel)
...
function Jutul.select_minimum_output_variables!(
    out, system::Electrolyte, model::SimulationModel
)
...
@inline function computeElectrolyteConductivity_Xu2015(c::Real, T::Real)
    ...
    conductivity =
        ...
        1.2544 *
        (
            -8.2488 + 0.053248 * T - 2.987e-5 * (T^2) + 0.26235e-3 * c - 9.3063e-6 * c * T +
            8.069e-9 * c * T^2 +
...
@jutul_secondary(function update_dmudc!(dmudc, dmudc_def::DmuDc, model, Temperature, C, ix)
    R = GAS_CONSTANT
    ...
    function update_conductivity!(
        ...
        for i in ix
            ...
            if haskey(model.system.params, :conductivity_data)
                @inbounds kappa[i] =
                    model.system[:conductivity_func](C[i]) * VolumeFraction[i]^b

            else
                @inbounds kappa[i] =
                    model.system[:conductivity_func](C[i], Temperature[i]) *
                    VolumeFraction[i]^b
            end
        end
    end
...
@jutul_secondary function update_diffusivity!(
    ...
    for i in ix
        ...
        if haskey(model.system.params, :diffusivity_data)
            ...
            @inbounds D[i] =
                model.system[:diffusivity_func](C[i], Temperature[i]) * VolumeFraction[i]^b
        end
    end
end

@jutul_secondary function update_chem_coef!(
    chemCoef, tv::ChemCoef, model::ElectrolyteModel, Conductivity, DmuDc, ix
)
...
function Jutul.face_flux!(
    ::T,
    c,
    other,
    face,
    face_sign,
    eq::ConservationLaw{:Charge,<:Any},
    state,
    model::ElectrolyteModel,
    dt,
    flow_disc,
