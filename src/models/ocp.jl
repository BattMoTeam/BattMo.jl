...
function computeOCP_Graphite_Torchio(c, T, cmax)
    ...
    refT = 298.15
    refOCP = (
        0.7222 + 0.1387 * theta + 0.0290 * theta^0.5 - 0.0172 / theta +
        0.0019 / theta^1.5 +
        0.2808 * exp(0.9 - 15.0 * theta) - 0.7984 * exp(0.4465 * theta - 0.4108)
    )
...
function computeOCP_Graphite_SiOx_Chen2020(c, T, cmax)
    ...
    ocp =
        1.9793 * exp(-39.3631 * x) + 0.2482 - 0.0909 * tanh(29.8538 * (x - 0.1234)) -
...
const coeff1_refOCP_nmc111 = Polynomial([
    -4.656, 0, +88.669, 0, -401.119, 0, +342.909, 0, -462.471, 0, +433.434
]);

const coeff2_refOCP_nmc111 = Polynomial([
    -1, 0, +18.933, 0, -79.532, 0, +37.311, 0, -73.083, 0, +95.960
])

const coeff1_dUdT_nmc111 = Polynomial([
    0.199521039, -0.928373822, +1.364550689000003, -0.611544893999998
]);

const coeff2_dUdT_nmc111 = Polynomial([
    1, -5.661479886999997, +11.47636191, -9.82431213599998, +3.048755063
])
...
function computeOCP_LFP_Gerver2011(c, T, cmax)
    ocp =
        3.41285712e+00 - 1.49721852e-02 * c / cmax +
