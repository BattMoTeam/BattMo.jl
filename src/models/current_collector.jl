...
struct CurrentCollector{T,D} <:
       ElectroChemicalComponent where {T<:CurrentCollectorParameters,D<:AbstractDict}
    params::T
...
function Jutul.select_minimum_output_variables!(
    out, system::CurrentCollector, model::SimulationModel
)
    push!(out, :Charge)
end
...
function Jutul.select_parameters!(S, system::CurrentCollector, model::SimulationModel)
...
function Jutul.select_equations!(eqs, system::CurrentCollector, model::SimulationModel)
