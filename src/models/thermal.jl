...
function Jutul.select_minimum_output_variables!(
    out, system::Thermal, model::SimulationModel
)
    push!(out, :Temperature)
end

function Jutul.select_primary_variables!(S, system::Thermal, model::SimulationModel)
    S[:Temperature] = Temperature()
end

function Jutul.select_secondary_variables!(S, system::Thermal, model::SimulationModel)
    # S[:TPkGrad_Phi] = TPkGrad{Phi}()
    S[:Energy] = Energy()
end

@jutul_secondary function update_as_secondary!(acc, tv::Energy, model, Temperature, ix)
    for i in ix
        @inbounds acc[i] = Temperature[i]
    end
end

function Jutul.select_parameters!(S, system::Thermal, model::SimulationModel)
...
function Jutul.select_equations!(eqs, system::Thermal, model::SimulationModel)
