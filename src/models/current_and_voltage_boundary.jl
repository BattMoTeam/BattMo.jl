export CurrentAndVoltageSystem,
...
CurrentAndVoltageModel{P} = SimulationModel{
    CurrentAndVoltageDomain,CurrentAndVoltageSystem{P}
}
...
mutable struct SimpleCVPolicy{R} <: AbstractCVPolicy
    ...
    Imax::R
    voltage::R
    function SimpleCVPolicy(;
        current_function=missing, Imax::T=0.0, voltage=missing
    ) where {T<:Real}
...
function CyclingCVPolicy(
    ...
    tolerances = (cc_discharge1=1e-4, cc_discharge2=0.9, cc_charge1=1e-4, cv_charge2=0.9)
...
function Jutul.select_primary_variables!(
    S, system::CurrentAndVoltageSystem, model::SimulationModel
)
    S[:Phi] = VoltageVar()
...
function Jutul.select_equations!(
    eqs, system::CurrentAndVoltageSystem, model::SimulationModel
)
...
function Jutul.select_parameters!(
    S, system::CurrentAndVoltageSystem{SimpleCVPolicy{R}}, model::SimulationModel
) where {R}
    S[:ImaxDischarge] = ImaxDischarge()
end

function Jutul.select_parameters!(
    S, system::CurrentAndVoltageSystem{CyclingCVPolicy{R,I}}, model::SimulationModel
) where {R,I}
    S[:ImaxDischarge] = ImaxDischarge()
...
function Jutul.select_minimum_output_variables!(
    outputs, system::CurrentAndVoltageSystem{R}, model::SimulationModel
) where {R}
...
function setup_initial_control_policy!(
    policy::SimpleCVPolicy, inputparams::InputParams, parameters
)
...
function setup_initial_control_policy!(
    policy::CyclingCVPolicy, inputparams::InputParams, parameters
)
...
function Jutul.update_primary_variable!(
    state, p::CurrentVar, state_symbol, model::P, dx, w
) where {R,I,Q<:CyclingCVPolicy{R,I},P<:CurrentAndVoltageModel{Q}}
...
function check_constraints(model, storage)
    ...
    if (ctrlType == ctrlType0 && rsw.afterSwitchRegion) ||
        (ctrlType == nextCtrlType && !rswN.beforeSwitchRegion)
        arefulfilled = false
    end
...
"""
function Jutul.perform_step_solve_impl!(
    report,
    storage,
    model::MultiModel{:Battery,T},
    config,
    dt,
    iteration,
    rec,
...
function Jutul.reset_state_to_previous_state!(
    ...
    model::SimulationModel{
        CurrentAndVoltageDomain,CurrentAndVoltageSystem{CyclingCVPolicy{T1,T2}},T3,T4
    },
) where {T1,T2,T3,T4}
    invoke(
...
function Jutul.update_equation_in_entity!(
    v, i, state, state0, eq::ControlEquation, model, dt, ldisc=local_discretization(eq, i)
)
    I = only(state.Current)
    phi = only(state.Phi)
...
function Jutul.update_equation_in_entity!(
    v, i, state, state0, eq::CurrentEquation, model, dt, ldisc=local_discretization(eq, i)
)
...
"""
function Jutul.update_after_step!(
    storage,
    domain::CurrentAndVoltageDomain,
    model::CurrentAndVoltageModel,
    dt,
    forces;
    time=NaN,
)
    ...
    if policy isa CyclingCVPolicy
        ...
        if initctrl == charging
            if (ctrlType0 == cc_discharge1 || ctrlType0 == cc_discharge2) &&
                (ctrlType == cc_charge1 || ctrlType == cv_charge2)
                ncycles = ncycles + 1
            end

        elseif initctrl == discharging
            if (ctrlType0 == cc_charge1 || ctrlType0 == cv_charge2) &&
                (ctrlType == cc_discharge1 || ctrlType == cc_discharge2)
                ncycles = ncycles + 1
            end
        end
