...
export BCCurrent
export TPFAInterfaceFluxCT,
    ButlerVolmerActmatToElyteCT, ButlerVolmerElyteToActmatCT, ButlerVolmerInterfaceFluxCT
...
const ElectroChemicalComponentModel = SimulationModel{
    <:Any,<:ElectroChemicalComponent,<:Any,<:Any
}
...
const BCCurrent = Dict(:Charge => :BCCharge, :Mass => :BCMass, :Energy => :BCCurrent)
...
struct MinimalECTPFAGrid{V,N,B,BT,M} <: ElectroChemicalGrid
    ...
    function MinimalECTPFAGrid(pv, N, T; bc_cells=[], bc_hfT=[], P=[], S=[], vf=[])
        ...
        return new{typeof(pv),typeof(N),typeof(bc_cells),typeof(bc_hfT),typeof(P)}(
            pv, N, bc_cells, bc_hfT, P, S, vf, T
        )
    end
end
...
function Base.show(io::IO, g::MinimalECTPFAGrid)
    print(
        io, "MinimalECTPFAGrid ($(number_of_cells(g)) cells, $(number_of_faces(g)) faces)"
    )
end
################
# Constructors #
...
function Jutul.default_parameter_values(
    d::DataDomain, model::SimulationModel{O,S,F,C}, ::ECTransmissibilities, symb
) where {G<:MinimalECTPFAGrid,D,E,M,O<:DiscretizedDomain{G,D,E,M},S,F,C}
...
function Jutul.default_parameter_values(
    d::DataDomain, model::SimulationModel{O,S,F,C}, ::Volume, symb
) where {G<:MinimalECTPFAGrid,D,E,M,O<:DiscretizedDomain{G,D,E,M},S,F,C}
    repG = physical_representation(model)
    return repG.volumes
end
...
function Jutul.default_parameter_values(
    d::DataDomain, model::SimulationModel{O,S,F,C}, ::VolumeFraction, symb
) where {G<:MinimalECTPFAGrid,D,E,M,O<:DiscretizedDomain{G,D,E,M},S,F,C}
    repG = physical_representation(model)
...
function BatteryGeneralPreconditioner()
    ...
    push!(
        varpreconds,
        VariablePrecond(
            Jutul.AMGPreconditioner(:ruge_stuben), :Phi, :charge_conservation, nothing
        ),
    )
...
function BatteryCPhiPreconditioner(
    c_precond=Jutul.AMGPreconditioner(:ruge_stuben),
