export ModelConfigured, BatteryModel
export _setup_active_material, _setup_electrolyte, _setup_ne_current_collector, _setup_pe_current_collector
export _setup_model, _setup_submodels, _setup_multimodel
export _setup_coupling_cross_terms!, _setup_volume_fractions!
export _setup_initial_state
...
abstract type BatteryModel <: ModelConfigured end


function _setup_model(model::B, input, grids, couplings; kwargs...) where {B <: BatteryModel}
...
function _setup_submodels(model::B, input, grids, couplings; kwargs...) where {B <: BatteryModel}
...
function _setup_multimodel(model::B, submodels, input) where {B <: BatteryModel}
...
function _setup_volume_fractions!(model::B, grids, coupling) where {B <: BatteryModel}
...
function _setup_electrolyte(model::B, input, grids) where {B <: BatteryModel}
	params = JutulStorage()
...
function _setup_ne_current_collector(model::B, input, grids, couplings) where {B <: BatteryModel}
...
function _setup_pe_current_collector(model::B, input, grids, couplings) where {B <: BatteryModel}
...
"""
	...
	"""
function _setup_active_material(model::B, name::Symbol, input, grids, couplings) where {B <: BatteryModel}
...
function _set_parameters(model::B, input) where {B <: BatteryModel}
	multimodel = model.multimodel
	cycling_protocol = input.cycling_protocol
	cell_parameters = input.cell_parameters
...
function _setup_coupling_cross_terms!(model::B,
	parameters::Dict{Symbol, <:Any},
	couplings) where {B <: BatteryModel}
...
function _setup_initial_state(input, model::B) where {B <: BatteryModel}
