export fluid_volume, half_face_two_point_kgrad
...
@jutul_secondary function update_as_secondary!(acc, tv::Charge, model, Phi, ix)
    for i in ix
        @inbounds acc[i] = 0.0
    end
end
...
function Jutul.apply_boundary_conditions!(
    storage, parameters, model::ElectroChemicalComponentModel
)
    ...
    if dobc
        ...
        if dolegacy
            ...
            for (i, c) in enumerate(bc)
                @inbounds acc[c] +=
                    conductivity[c] * T_hf[i] * (Phi[c] - value(BoundaryPhi[i]))
            end
        else
...
function Jutul.select_parameters!(
    prm,
    D::Union{TwoPointPotentialFlowHardCoded,PotentialFlow},
    model::ElectroChemicalComponentModel,
)
...
function Jutul.select_parameters!(
    prm, D::MinimalECTPFAGrid, model::ElectroChemicalComponentModel
)
    prm[:Volume] = Volume()
