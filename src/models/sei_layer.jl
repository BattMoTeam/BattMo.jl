...
function Jutul.select_primary_variables!(S, system::ActiveMaterialP2D, model::SEImodel)
    S[:Phi] = Phi()
    S[:Cp] = Cp()
    S[:Cs] = Cs()
...
function Jutul.select_secondary_variables!(S, system::ActiveMaterialP2D, model::SEImodel)
    S[:Charge] = Charge()
    S[:Ocp] = Ocp()
...
function Jutul.select_minimum_output_variables!(
    outputs, system::ActiveMaterialP2D, model::SEImodel
)
...
function Jutul.select_equations!(eqs, system::ActiveMaterialP2D, model::SEImodel)
...
function Jutul.update_equation_in_entity!(
    eq_buf, self_cell, state, state0, eq::SEImassCons, model, dt, ldisc=nothing
)
    # do nothing
end

function Jutul.update_equation_in_entity!(
    eq_buf, self_cell, state, state0, eq::SEIvoltageDropEquation, model, dt, ldisc=nothing
)
    # do nothing    
end
...
function apply_bc_to_equation!(
    storage, parameters, model::SEImodel, eq::SEIvoltageDropEquation, eq_s
)
    # do nothing
end
...
@jutul_secondary(
    function update_vocp!(
        SEIlength, tv::SEIlength, model::SEImodel, normalizedSEIlength, ix
    )
...
@jutul_secondary(
    function update_vocp!(
        SEIvoltageDrop, tv::SEIvoltageDrop, model::SEImodel, normalizedSEIvoltageDrop, ix
    )
