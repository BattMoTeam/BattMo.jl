...
struct ActiveMaterial{label,D,T,Di} <: AbstractActiveMaterial{
    label
} where {D<:SolidDiffusionDiscretization,T<:ActiveMaterialParameters,Di<:AbstractDict}
    params::T
...
function ActiveMaterialP2D(
    ...
    return ActiveMaterialP2D{label,typeof(discretization),typeof(params),typeof(scalings)}(
        params, discretization, scalings
    )
end

## Create ActiveMaterial with no solid diffusion
function ActiveMaterialNoParticleDiffusion(
    params::ActiveMaterialParameters, scalings=Dict()
)
    discretization = NoParticleDiffusion()
    params = convert_to_immutable_storage(params)
    return ActiveMaterialNoParticleDiffusion{
        NoParticleDiffusion,typeof(params),typeof(scalings)
    }(
        params, discretization, scalings
    )
end
...
function setupSolidDiffusionDiscretization(rp, N, D)
    ...
    dr = rp / N
    rc = [dr * (i - 1 / 2) for i in 1:N]
    rf = [dr * i for i in 0:(N + 1)]
    ...
    data = Dict(:N => N, :D => D, :rp => rp, :hT => hT, :vols => vols, :div => div)
...
function Jutul.select_primary_variables!(
    S, system::ActiveMaterialP2D, model::SimulationModel
)
    S[:Phi] = Phi()
    S[:Cp] = Cp()
    S[:Cs] = Cs()
...
function Jutul.degrees_of_freedom_per_entity(model::ActiveMaterialModel, ::Cp)
    return solid_diffusion_discretization_number(model.system)
end

function Jutul.degrees_of_freedom_per_entity(model::ActiveMaterialModel, ::SolidDiffFlux)
    return solid_diffusion_discretization_number(model.system) - 1
end

function Jutul.select_parameters!(S, system::ActiveMaterialP2D, model::SimulationModel)
...
function Jutul.select_equations!(eqs, system::ActiveMaterialP2D, model::SimulationModel)
...
function Jutul.select_minimum_output_variables!(
    out, system::ActiveMaterialP2D, model::SimulationModel
)
    push!(out, :Charge)
    push!(out, :Ocp)
...
@jutul_secondary(
    function update_solid_diffusion_flux!(
        SolidDiffFlux,
        tv::SolidDiffFlux,
        model::SimulationModel{<:Any,ActiveMaterialP2D{label,D,T,Di},<:Any,<:Any},
        Cp,
...
function Jutul.update_equation_in_entity!(
    eq_buf, self_cell, state, state0, eq::SolidMassCons, model, dt, ldisc=Nothing
)
...
function Jutul.update_equation_in_entity!(
    eq_buf, self_cell, state, state0, eq::SolidDiffusionBc, model, dt, ldisc=Nothing
)
    disc = model.system.discretization
    N = disc[:N]
    hT = disc[:hT]
...
function Jutul.select_primary_variables!(
    S, system::ActiveMaterialNoParticleDiffusion, model::SimulationModel
)
    S[:Phi] = Phi()
    S[:C] = C()
end
...
function Jutul.select_equations!(
    eqs, system::ActiveMaterialNoParticleDiffusion, model::SimulationModel
)
    disc = model.domain.discretizations.charge_flow
    eqs[:charge_conservation] = ConservationLaw(disc, :Charge)
...
function Jutul.select_minimum_output_variables!(
    out, system::ActiveMaterialNoParticleDiffusion, model::SimulationModel
)
    push!(out, :Charge)
    push!(out, :Mass)
    push!(out, :Ocp)
