...
function Jutul.update_equation_in_entity!(v, i, state, state0, eq::ControlEquation, model, dt, ldisc = local_discretization(eq, i))
    ...
    return if ctrl isa GenericController
...
function Jutul.initialize_extra_state_fields!(state, ::Any, model::ExternalCircuitModel; T = Float64)
    ...
    # return if protocol isa SimpleCVPolicy
    ...
    return if protocol isa FunctionProtocol
    ...
    elseif protocol isa GenericProtocol
        ...
        current = 0.0
        voltage = 0.0
        target = get_initial_current(step)
        state[:Controller] = GenericController(protocol, step, step_count, step_index, cycle_count, time_in_step, current, voltage, protocol.initial_state_of_charge; target, T)
...
function compute_state_of_charge(
        previous_state_of_charge::Real,
        dt::Real,
        rated_capacity::Real,
        current::Real;
        η_chg::Real = 1.0,
...
function Jutul.reset_state_to_previous_state!(
        storage,
        model::SimulationModel{ExternalCircuitDomain, ExternalCircuitSystem{InputCurrentProtocol{T1}}, T3, T4}
    ) where {T1, T3, T4}
...
function update_controller!(state, state0, protocol::AbstractProtocol, dt)
    ...
    return if protocol isa GenericProtocol
