...
init = BattMo.MatlabFile(((@__DIR__)* "/test_model_cc_04.mat"),state_ref=false)
use_p2d=false
use_groups=false
extra_timing=false
max_step=nothing
...
t = cumsum(timesteps)
###################################################################################################################################
#Setup and run sim using JuliaDifferentialEquations
###################################################################################################################################
...
#Determine differential variables. Needed by IDA to find initial condition
ind = SparseArrays.findnz(m_jac)[1] #We assume m to be linear in u
vars= zeros(Integer,length(x0))
vars[ind].=1

#Residual + jacobian functions
p_obj = ODEParam(sim,forces) # See state_variable_mapping
...
@time begin
    ret = solve(prob,IDA(linear_solver=:KLU)) # Take a look at the documentation!
end
