...
function setup_initial_control_policy!(
    policy::SimpleCVPolicy, inputparams::MatlabInputParams, parameters
)
...
function setup_initial_control_policy!(
    policy::CyclingCVPolicy, inputparams::MatlabInputParams, parameters
)
...
function setup_timesteps(
    inputparams::MatlabInputParams; max_step::Union{Integer,Nothing}=nothing, kwarg...
)
    """
...
function setup_coupling_cross_terms!(
    ...
    if include_cc
        ...
        srange = Int64.(
            exported_all["model"]["NegativeElectrode"]["couplingTerm"]["couplingcells"][
                :, 1
            ],
        )
        trange = Int64.(
        ...
        couplingfaces = Int64.(
            exported_all["model"]["NegativeElectrode"]["couplingTerm"]["couplingfaces"]
        )
        couplingcells = Int64.(
            exported_all["model"]["NegativeElectrode"]["couplingTerm"]["couplingcells"]
        )
        ...
        target = Dict(:model => :PeAm, :equation => :charge_conservation)
        source = Dict(:model => :PeCc, :equation => :charge_conservation)
        srange = Int64.(
            exported_all["model"]["PositiveElectrode"]["couplingTerm"]["couplingcells"][
                :, 1
            ],
        )
        trange = Int64.(
    ...
    trans = getHalfTrans(
        msource, couplingfaces, couplingcells, "effectiveElectronicConductivity"
    )

    ct = TPFAInterfaceFluxCT(trange, srange, trans)
    ct_pair = setup_cross_term(
        ct; target=component, source=:Control, equation=:charge_conservation
    )
    add_cross_term!(model, ct_pair)
    ...
    ct_pair = setup_cross_term(
        ct; target=:Control, source=component, equation=:charge_conservation
    )
    add_cross_term!(model, ct_pair)
end
...
function include_current_collectors(inputparams::MatlabInputParams)
    ...
    if haskey(model, "include_current_collectors")
        if isempty(model["include_current_collectors"])
            include_cc = false
        elseif isa(model["include_current_collectors"], Bool) &&
...
function setup_submodels(
    ...
    function setup_component(obj::Dict, sys, general_ad::Bool, bcfaces=nothing)
        ...
        model = SimulationModel(
            domain, sys; context=DefaultContext(), data_domain=data_domain
        )
        return model
    end
    ...
    function setup_active_material(name::Symbol, general_ad::Bool)
        ...
        if !include_cc && name == :NeAm
            bcfaces = convert_to_int_vector(
                inputparams_co["externalCouplingTerm"]["couplingfaces"]
            )
    ...
    if include_cc
        ...
        bcfaces = convert_to_int_vector(
            inputparams_necc["externalCouplingTerm"]["couplingfaces"]
        )
    ...
    model_elyte = setup_component(inputparams["Electrolyte"], elyte, general_ad, nothing)
...
function setup_battery_parameters(inputparams::MatlabInputParams, model::MultiModel)
    parameters = Dict{Symbol,Any}()
...
function setup_initial_state(inputparams::MatlabInputParams, model::MultiModel)
    exported = inputparams.all
...
function exported_model_to_domain(exported; bcfaces=nothing, general_ad=true)
    ...
    G = MinimalECTPFAGrid(
        volumes, N, vec(T); bc_cells=bc_cells, bc_hfT=bc_hfT, P=P, S=S, vf=vf
    )
