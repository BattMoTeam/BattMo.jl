...
export run_battery, setup_simulation, setup_model, Simulation, solve
...
struct Simulation <: SolvingProblem
    ...
    function Simulation(
        ...
        if model.is_valid
            ...
            cell_parameters_is_valid = validate_parameter_set(
                cell_parameters, model_settings
            )
            cycling_protocol_is_valid = validate_parameter_set(cycling_protocol)
            simulation_settings_is_valid = validate_parameter_set(
                simulation_settings, model_settings
            )

            if cell_parameters_is_valid &&
...
function solve(problem::Simulation; accept_invalid=false, hook=nothing, kwargs...)
    if accept_invalid == true
        output = problem.function_to_solve(
            problem.model,
...
function run_battery(
    ...
    battmo_formatted_input = convert_parameter_sets_to_battmo_input(
        model_settings, cell_parameters, cycling_protocol, simulation_settings
    )
...
"""
function run_battery(inputparams::BattMoFormattedInput; hook=nothing, kwargs...)
    """
    ...
    if !isnothing(hook)
        hook(simulator, model, state0, forces, timesteps, cfg)
    end
...
function setup_simulation(
    ...
    model, parameters = setup_model(
        inputparams; use_groups=use_groups, general_ad=general_ad, model_kwargs...
    )
...
function setup_model(
    ...
    model, couplings = setup_submodels(
        inputparams;
...
function setup_submodels(
    ...
    stringNames = Dict(:NeAm => "NegativeElectrode", :PeAm => "PositiveElectrode")
    ...
    function setup_active_material(name::Symbol; kwargs...)
        ...
        am_params[:reaction_rate_constant_func] =
            (c, T) -> compute_reaction_rate_constant(c, T, k0, Eak)
        ...
        model_am = setup_component(
            grid, sys_am; general_ad=general_ad, dirichletBoundary=boundary, kwargs...
        )
    ...
    if include_cc
        ...
        model_necc = setup_component(
            grid, sys_necc; dirichletBoundary=boundary, general_ad=general_ad, kwargs...
        )
    end
    ...
    if haskey(inputparams_elyte["diffusionCoefficient"], "function")
        exp = setup_diffusivity_evaluation_expression_from_string(
            inputparams_elyte["diffusionCoefficient"]["function"]
        )
    ...
    if haskey(inputparams_elyte["ionicConductivity"], "function")
        exp = setup_conductivity_evaluation_expression_from_string(
            inputparams_elyte["ionicConductivity"]["function"]
        )
    ...
    model_elyte = setup_component(
        grids["Electrolyte"], elyte; general_ad=general_ad, kwargs...
    )
    ...
    if include_cc
        ...
        model_pecc = setup_component(grid, sys_pecc; general_ad=general_ad, kwargs...)
    end
    ...
    if !include_cc
        groups = nothing
        model = MultiModel(
            (NeAm=model_neam, Elyte=model_elyte, PeAm=model_peam, Control=model_control),
            Val(:Battery);
            groups=groups,
        )
    else
        ...
        model = MultiModel(models, Val(:Battery); groups=groups, reduction=reduction)
    end

    setup_volume_fractions!(model, grids, couplings["Electrolyte"])

    output = (model=model, couplings=couplings, grids=grids)
...
function setup_component(
    grid::FiniteVolumeMesh, sys; general_ad::Bool=true, dirichletBoundary=nothing, kwargs...
)
...
function setup_battery_parameters(inputparams::InputParams, model::MultiModel)
    ...
    prm_neam[:Conductivity] = computeEffectiveConductivity(
        model[:NeAm], inputparams["NegativeElectrode"]["Coating"]
    )
    ...
    prm_peam[:Conductivity] = computeEffectiveConductivity(
        model[:PeAm], inputparams["PositiveElectrode"]["Coating"]
    )
...
function setup_initial_state(inputparams::InputParams, model::MultiModel)
    include_cc = include_current_collectors(model)
...
function setup_coupling_cross_terms!(
    inputparams::InputParams, model::MultiModel, parameters::Dict{Symbol,<:Any}, couplings
)
    ...
    if discretisation_type(model[:NeAm]) == :P2Ddiscretization
        ...
        ct_pair = setup_cross_term(
            ct; target=:Elyte, source=:NeAm, equation=:charge_conservation
        )
        add_cross_term!(model, ct_pair)
        ct_pair = setup_cross_term(
            ct; target=:Elyte, source=:NeAm, equation=:mass_conservation
        )
        add_cross_term!(model, ct_pair)

        ct = ButlerVolmerElyteToActmatCT(srange, trange)
        ct_pair = setup_cross_term(
            ct; target=:NeAm, source=:Elyte, equation=:charge_conservation
        )
        add_cross_term!(model, ct_pair)
        ct_pair = setup_cross_term(
            ct; target=:NeAm, source=:Elyte, equation=:solid_diffusion_bc
        )
        add_cross_term!(model, ct_pair)

        if model[:NeAm] isa SEImodel
            ct_pair = setup_cross_term(
                ct; target=:NeAm, source=:Elyte, equation=:sei_mass_cons
            )
            add_cross_term!(model, ct_pair)
            ct_pair = setup_cross_term(
                ct; target=:NeAm, source=:Elyte, equation=:sei_voltage_drop
            )
            add_cross_term!(model, ct_pair)
        end
    ...
    if discretisation_type(model[:PeAm]) == :P2Ddiscretization
        ct = ButlerVolmerActmatToElyteCT(trange, srange)
        ct_pair = setup_cross_term(
            ct; target=:Elyte, source=:PeAm, equation=:charge_conservation
        )
        add_cross_term!(model, ct_pair)
        ct_pair = setup_cross_term(
            ct; target=:Elyte, source=:PeAm, equation=:mass_conservation
        )
        add_cross_term!(model, ct_pair)

        ct = ButlerVolmerElyteToActmatCT(srange, trange)
        ct_pair = setup_cross_term(
            ct; target=:PeAm, source=:Elyte, equation=:charge_conservation
        )
        add_cross_term!(model, ct_pair)
        ct_pair = setup_cross_term(
            ct; target=:PeAm, source=:Elyte, equation=:solid_diffusion_bc
        )
        add_cross_term!(model, ct_pair)
    ...
    if include_cc
        ...
        srange_cells = collect(
            couplings["NegativeCurrentCollector"]["NegativeElectrode"]["cells"]
        )
        trange_cells = collect(
            couplings["NegativeElectrode"]["NegativeCurrentCollector"]["cells"]
        )

        srange_faces = collect(
            couplings["NegativeCurrentCollector"]["NegativeElectrode"]["faces"]
        )
        trange_faces = collect(
            couplings["NegativeElectrode"]["NegativeCurrentCollector"]["faces"]
        )
        ...
        trans = getTrans(
            msource, mtarget, couplingfaces, couplingcells, psource, ptarget, :Conductivity
        )
        @assert size(trans, 1) == size(srange_cells, 1)
        ct = TPFAInterfaceFluxCT(trange_cells, srange_cells, trans)
        ct_pair = setup_cross_term(
            ct; target=:NeAm, source=:NeCc, equation=:charge_conservation
        )
        add_cross_term!(model, ct_pair)
        ct = TPFAInterfaceFluxCT(srange_cells, trange_cells, trans)
        ct_pair = setup_cross_term(
            ct; target=:NeCc, source=:NeAm, equation=:charge_conservation
        )
        add_cross_term!(model, ct_pair)
        ...
        srange_cells = collect(
            couplings["PositiveCurrentCollector"]["PositiveElectrode"]["cells"]
        )
        trange_cells = collect(
            couplings["PositiveElectrode"]["PositiveCurrentCollector"]["cells"]
        )

        srange_faces = collect(
            couplings["PositiveCurrentCollector"]["PositiveElectrode"]["faces"]
        )
        trange_faces = collect(
            couplings["PositiveElectrode"]["PositiveCurrentCollector"]["faces"]
        )
        ...
        ct = TPFAInterfaceFluxCT(trange_cells, srange_cells, trans)
        ct_pair = setup_cross_term(
            ct; target=:PeAm, source=:PeCc, equation=:charge_conservation
        )
        add_cross_term!(model, ct_pair)

        ct = TPFAInterfaceFluxCT(srange_cells, trange_cells, trans)
        ct_pair = setup_cross_term(
            ct; target=:PeCc, source=:PeAm, equation=:charge_conservation
        )
        add_cross_term!(model, ct_pair)
    end
    ...
    ct_pair = setup_cross_term(
        ct; target=controlComp, source=:Control, equation=:charge_conservation
    )
    add_cross_term!(model, ct_pair)

    ct = AccumulatorInterfaceFluxCT(1, trange, trans)
    ct_pair = setup_cross_term(
        ct; target=:Control, source=controlComp, equation=:charge_conservation
    )
    add_cross_term!(model, ct_pair)
...
function get_scalings(model, parameters)
    ...
    scaling = (
        model_label=:Elyte,
        equation_label=:charge_conservation,
        value=F * volRefs[:Elyte] * RvolRef,
    )
    push!(scalings, scaling)

    scaling = (
        model_label=:Elyte,
        equation_label=:mass_conservation,
        value=volRefs[:Elyte] * RvolRef,
    )
    push!(scalings, scaling)
    ...
    for elde in eldes
        ...
        if include_current_collectors(model)
            ...
            scaling = (
                model_label=cc,
                equation_label=:charge_conservation,
                value=F * coef[1] * volRefs[elde] * RvolRef,
            )
            push!(scalings, scaling)
        end
        ...
        if model[elde] isa SEImodel
            ...
            scaling = (
                model_label=elde, equation_label=:sei_voltage_drop, value=SEIvoltageDropRef
            )
            push!(scalings, scaling)
...
function setup_timesteps(inputparams::InputParams; kwargs...)
    """
    ...
    if controlPolicy == "CCDischarge"
        ...
        if haskey(inputparams["TimeStepping"], "useRampup") &&
            inputparams["TimeStepping"]["useRampup"]
...
function setup_config(
    ...
    if model[:Control].system.policy isa CyclingCVPolicy
        cfg[:tolerances][:global_convergence_check_function] =
            (model, storage) -> check_constraints(model, storage)

        function post_hook(done, report, sim, dt, forces, max_iter, cfg)
            s = get_simulator_storage(sim)
            m = get_simulator_model(sim)

            if s.state.Control.ControllerCV.numberOfCycles >=
                m[:Control].system.policy.numberOfCycles
                report[:stopnow] = true
            else
...
function getTrans(
    model1::Dict{String,<:Any}, model2::Dict{String,Any}, faces, cells, quantity::String
)
...
function getHalfTrans(model::SimulationModel, bcfaces, bccells, parameters, quantity)
...
function include_current_collectors(inputparams::InputParams)
    ...
    if haskey(jsondict, "include_current_collectors") &&
        !jsondict["include_current_collectors"]
        include_cc = false
    else
        include_cc = true
    end
