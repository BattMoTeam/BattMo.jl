export pouch_grid,
    find_coupling, find_common, findBoundary, convert_geometry, one_dimensional_grid
...
function setup_couplings(components, grids, global_maps)
    ...
    for (ind1, comp1) in enumerate(components)
        ...
        for (ind2, comp2) in enumerate(components)
            intersection = find_coupling(
                global_maps[comp1], global_maps[comp2], [comp1, comp2]
            )
            ...
            if ind1 != ind2
                ...
                if isnothing(cells)
                    ...
                    if !isnothing(faces)
                        ...
                        intersection_tmp = Dict(
                            "cells" => loccells, "faces" => locfaces, "face_type" => true
                        )
                    end
                    ...
                    if size(faces, 1) != size(cells, 1)
                        intersection_tmp = Dict(
                            "cells" => cells[:, 1], "faces" => [], "face_type" => false
                        )
                    else
                        @assert false
                    end
...
function convert_geometry(grids, couplings; include_current_collectors=true)
    if include_current_collectors
        ...
        components = ["NegativeElectrode", "Separator", "PositiveElectrode", "Electrolyte"]
    end
                ...
                if coupling["face_type"]
                    ...
                    for fi in eachindex(faces)
                        ...
                        for lfi in eachindex(candidates)
                            fnodes = Set(
                                ugrid.boundary_faces.faces_to_nodes[candidates[lfi]]
                            )
                            if fnodes == lnodes
...
function one_dimensional_grid(geomparams::InputGeometryParams)
    ...
    vars = ["thickness", "N"]
    vals = Dict("thickness" => Vector{Float64}(), "N" => Vector{Int}())
    ...
    if include_current_collectors
        ...
        components = ["NegativeElectrode", "Separator", "PositiveElectrode"]
    ...
    grids, couplings = convert_geometry(
        grids, couplings; include_current_collectors=include_current_collectors
    )
    ...
    if include_current_collectors
        boundaryComponents = Dict(
            "left" => "NegativeCurrentCollector", "right" => "PositiveCurrentCollector"
        )
    else
        boundaryComponents = Dict(
            "left" => "NegativeElectrode", "right" => "PositiveElectrode"
        )
    end
...
function pouch_grid(geomparams::InputGeometryParams)
    ...
    pe_endbox_list_of_iterators = [
        (Nx * (i * Ny - pe_tab_ny) + 1):(Nx * Ny * i) for i in 1:Nz
    ]
    ...
    pe_tab_horz_index = cat(
        [(Nx * i - pe_tab_nx + 1):(Nx * i) for i in 1:pe_tab_ny]...; dims=1
    )
    ...
    pe_tab_cells = cat(
        getindex.(
            pe_endbox_list_of_iterators[(end - pe_cc_nz + 1):end], [pe_tab_horz_index]
        )...;
        dims=1,
    )
    ...
    ne_endbox_list_of_operators = [
        (Nx * Ny * (i - 1) + 1):(Nx * (Ny * (i - 1) + ne_tab_ny)) for i in 1:Nz
    ]
    ne_extra_cells = cat(ne_endbox_list_of_operators...; dims=1)

    ne_tab_horz_index = cat(
        [(Nx * (i - 1) + 1):(Nx * (i - 1) + pe_tab_nx) for i in 1:ne_tab_ny]...; dims=1
    )

    if same_side
        ne_tab_cells = cat(
            getindex.(pe_endbox_list_of_iterators[1:ne_cc_nz], [ne_tab_horz_index])...;
            dims=1,
        )
        setdiff!(pe_extra_cells, ne_tab_cells)
    else
        ne_tab_cells = cat(
            getindex.(ne_endbox_list_of_operators[1:ne_cc_nz], [ne_tab_horz_index])...;
            dims=1,
        )
    ...
    couplings["NegativeCurrentCollector"]["External"] = Dict(
        "cells" => bccells, "boundaryfaces" => bcfaces
    )
    ...
    couplings["PositiveCurrentCollector"]["External"] = Dict(
        "cells" => bccells, "boundaryfaces" => bcfaces
    )
