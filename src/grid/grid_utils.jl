...
function setup_tab_couplings(grids, inputparams, component)
    ...
    boundaryface_coupling = reduce(vcat, [get_tab_faces(first_tab_face) for first_tab_face in first_tab_faces])
    cell_coupling = geo.boundary_neighbors[boundaryface_coupling]
    return boundaryface_coupling, cell_coupling
