...
function convert_to_mrst_grid(g)
    ...
    cells_facePos =
        diff(g.faces.cells_to_faces.pos) + diff(g.boundary_faces.cells_to_faces.pos)

    begin
        faces_neighbors = findall(x -> 1 ∈ x, g.faces.neighbors)
        boundary_faces_neighbors =
            findall(x -> x == 1, g.boundary_faces.neighbors) .+ length(g.faces.neighbors)
    ...
    for i in 2:length(g.faces.cells_to_faces)
        ...
        boundary_faces_neighbors =
            findall(x -> x == i, g.boundary_faces.neighbors) .+ length(g.faces.neighbors)
        neighbors = add_order([faces_neighbors; boundary_faces_neighbors])
    ...
    G_raw_faces_neighbors = zeros(
        Int64, length(g.faces.neighbors) + length(g.boundary_faces.neighbors), 2
    )
    ...
    for i in 1:length(g.boundary_faces.neighbors)
        G_raw_faces_neighbors[i + length(g.faces.neighbors)] = findfirst(
            x -> i + length(g.faces.neighbors) ∈ x, G_raw_cells_faces[:, 1]
        )
    end

    G_raw_faces["nodes"] = reshape([c; d], :, 1)
    G_raw_faces["tag"] = zeros(
        length(g.faces.neighbors) + length(g.boundary_faces.neighbors)
    )
