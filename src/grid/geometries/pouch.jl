...
function pouch_grid(input)
	...
	neg_endbox = [
		(Nx*Ny*(i-1)+(neg_rows.start-1)*Nx+1):(Nx*Ny*(i-1)+neg_rows.stop*Nx)
		for i in 1:Nz
	]

	pos_endbox = [
		(Nx*Ny*(i-1)+(pos_rows.start-1)*Nx+1):(Nx*Ny*(i-1)+pos_rows.stop*Nx)
		for i in 1:Nz
	]
...
function compute_tab_indices_local(
	...
	get_idx_for_run = function (col_start, col_stop)
		...
		return vcat([(r - 1) * Nx .+ (col_start:col_stop) for r in rows]...)
	end
...
function compute_tab_columns(
	...
	for start in 1:(Nx-ncols+1)
		...
		better =
			(overlap > best_overlap + 1.0e-12) ||
			(abs(overlap - best_overlap) <= 1.0e-12 && center_distance < best_center_distance - 1.0e-12) ||
			(abs(overlap - best_overlap) <= 1.0e-12 && abs(center_distance - best_center_distance) <= 1.0e-12 && width_distance < best_width_distance - 1.0e-12)
