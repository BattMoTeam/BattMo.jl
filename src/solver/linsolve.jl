...
function modify_equation!(
    lsys, mass_cons_map, charge_cons_map, tfac, nc, context::ParallelCSRContext
)
    #NB not well stested
    vals = nonzeros(lsys.jac)
...
function Jutul.post_update_linearized_system!(
    lsys, executor, storage, model::MultiModel{:Battery}
)
    ...
    if (true)
        ...
        if isnothing(storage[:eq_maps].maps)
            mass_cons_map = setup_subset_equation_map(
                model, storage, e_models, :mass_conservation
            )
            #phi_map = setup_subset_residual_map(model, storage, e_models, :Phi)
            charge_cons_map = setup_subset_equation_map(
                model, storage, e_models, :charge_conservation
            )
            (mass_ind, charge_ind) = matrix_maps(
                lsys, mass_cons_map, charge_cons_map, context
            )
            storage[:eq_maps].maps = (
                mass_ind=mass_ind,
                charge_ind=charge_ind,
                mass_cons_map=mass_cons_map,
                charge_cons_map=charge_cons_map,
            )
        end
...
function Jutul.update_preconditioner!(
    prec::BatteryCPhiPreconditioner, lsys, context, model, storage, recorder, executor
)
    ...
    if isnothing(prec.data)
        ...
        mass_cons_map = setup_subset_equation_map(
            model, storage, c_models, :mass_conservation
        )
        # Phi part
        # p_models = nothing
        ...
        charge_cons_map = setup_subset_equation_map(
            model, storage, p_models, :charge_conservation
        )
        # @assert length(intersect(c_map, phi_map)) == 0
        nc = length(r)
        prec.data = (
            c=storage_chpi_precond(c_map),
    ...
    if !isnothing(prec.g_precond)
        update_preconditioner!(
            prec.g_precond, lsys, context, model, storage, recorder, executor
        )
    end
...
function setup_subset_residual_map(
    multi_model::MultiModel, storage, model_labels, variable_label
)
    M = []
...
function setup_subset_equation_map(
    multi_model::MultiModel, storage, model_labels, equation_label
)
    M = []
    offsets = get_submodel_offsets(storage)
    m_ix = 1
...
function battery_linsolve(
    model, method=:ilu0; rtol=0.001, solver=:gmres, verbose=0, kwarg...
)
    if method == :amg
        prec = amg_precond()
    elseif method == :ilu0
