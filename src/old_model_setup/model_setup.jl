...
function run_battery(
        model::ModelConfigured, cell_parameters::CellParameters, cycling_protocol::CyclingProtocol, simulation_settings::SimulationSettings;
        hook = nothing,
        use_p2d = true,
        kwargs...
    )
...
function get_simulation_input(problem::Simulation; kwargs...)
    ...
    inputparams = convert_parameter_sets_to_old_input_format(
        problem.model_setup.model_settings,
        problem.cell_parameters,
        problem.cycling_protocol,
        problem.simulation_settings
    )

    return output = get_simulation_input(inputparams; kwargs...)

end

function get_simulation_input(
        inputparams::BattMoInputFormatOld;
        use_p2d::Bool = true,
    ...
    model, parameters, couplings = setup_model!(
        inputparams;
...
function setup_battery_parameters(
        inputparams::InputParamsOld,
        model::MultiModel,
    )
...
function setup_coupling_cross_terms!(
    ...
    stringNames = Dict(
        :NegativeElectrodeCurrentCollector => "NegativeCurrentCollector",
        :NegativeElectrodeActiveMaterial => "NegativeElectrode",
        :PositiveElectrodeActiveMaterial => "PositiveElectrode",
    ...
    return add_cross_term!(model, ct1_pair)
...
function setup_volume_fractions!(model::MultiModel, grids, coupling)
    ...
    return model[:Electrolyte].domain.representation[:separator_volume_fraction] = vfseparator
