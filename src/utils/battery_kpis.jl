...
function compute_electrode_coating_mass(params::CellParameters, electrode::String)
    if !(electrode in ["PositiveElectrode", "NegativeElectrode"])
        error(
            "Electrode must be either PositiveElectrode or NegativeElectrode, not $electrode. Check for typos",
        )
    end
...
function compute_electrolyte_mass(params::CellParameters)
    ...
    pe_theoretical_density = compute_electrode_theoretical_density(
        params, "PositiveElectrode"
    )
    ...
    ne_theoretical_density = compute_electrode_theoretical_density(
        params, "NegativeElectrode"
    )
...
function compute_electrode_mass_loading(params::CellParameters, electrode::String)
    ...
    active_material_mass =
        electrode_mass*params[electrode]["ActiveMaterial"]["MassFraction"]
...
cell_parameters = read_cell_parameters(
    parameter_sets_directory * "cell_parameters/cell_parameter_set_chen2020.json"
)
println(cell_parameters["PositiveElectrode"]["ElectrodeCoating"]["Thickness"])

cycling_protocol = read_cycling_protocol(
    parameter_sets_directory * "cycling_protocols/CCCV.json"
)
println(cycling_protocol["Protocol"])

simulation_settings = read_simulation_settings(
    parameter_sets_directory * "model_settings/model_settings_P2D.json"
)
println(simulation_settings["ModelGeometry"])
