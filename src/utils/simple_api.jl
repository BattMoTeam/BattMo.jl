...
export extract_spatial_data, extract_output_times, extract_time_series_data
...
function get_output_time_series(output::NamedTuple, quantities::Vector{String})
	...
	selected_pairs = []
	available_quantities = ["Voltage", "Current", "CycleNumber"]

	voltage, current = extract_time_series_data(output)
	time = extract_output_times(output)
	states = output[:states]
	cycle_number = [state[:Control][:Controller].numberOfCycles for state in states]
	...
	for q in quantities
		...
		elseif q == "CycleNumber"
			push!(selected_pairs, :CycleNumber => cycle_number)
		else
