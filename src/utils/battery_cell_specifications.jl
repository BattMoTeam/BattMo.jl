...
function computeCellEnergy(states)
    ...
    time = [
        state[:Control][:ControllerCV].time for
        state in states if state[:Control][:Current][1] > 0
    ]
    E = [state[:Control][:Phi][1] for state in states if state[:Control][:Current][1] > 0]
    I = [
        state[:Control][:Current][1] for state in states if state[:Control][:Current][1] > 0
    ]
...
function computeCellMaximumEnergy(model::MultiModel; T=298.15, capacities=missing)
    ...
    if ismissing(capacities)
        capacities = NamedTuple([
            (name, computeElectrodeCapacity(model, name)) for name in eldes
        ])
    end
...
function computeCellSpecifications(model::MultiModel; T=298.15)
    capacities = (
        NeAm=computeElectrodeCapacity(model, :NeAm),
        PeAm=computeElectrodeCapacity(model, :PeAm),
    )
