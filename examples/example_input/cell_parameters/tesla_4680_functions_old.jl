...
# stoich_charge = (Q_charge - 3) ./ (maximum(Q_charge) .- minimum(Q_charge))
# stoich_discharge = (Q_discharge -3) ./ (minimum(Q_discharge) .- maximum(Q_discharge))
...
ne_ocp_interp = get_1d_interpolator(data_ne[:, 1], data_ne[:, 2])
# pe_ocp_interp = get_1d_interpolator(df_ocp_avg[!, "StoichiometricCoefficient"], df_ocp_avg[!, "PositiveElectrodeOCP"])
pe_ocp_interp = get_1d_interpolator(data_pe[:, 1], data_pe[:, 2])
