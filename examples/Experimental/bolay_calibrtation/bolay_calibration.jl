...
exp_file = joinpath(battmo_base, "examples/Experimental/resources/bolay_discharge_data.csv")
...
cycling_protocol["InitialTemperature"] = 293.15
...
free_calibration_parameter!(cal, ["PositiveElectrode", "ActiveMaterial", "MassFraction"]; lower_bound = 0.85, upper_bound = 0.98)
free_calibration_parameter!(cal, ["PositiveElectrode", "ConductiveAdditive", "MassFraction"]; lower_bound = 0.005, upper_bound = 0.10)
...
free_calibration_parameter!(cal, ["PositiveElectrode", "Coating", "EffectiveDensity"]; lower_bound = 2500.0, upper_bound = 4200.0)

# Negative electrode mass fractions
free_calibration_parameter!(cal, ["NegativeElectrode", "ActiveMaterial", "MassFraction"]; lower_bound = 0.85, upper_bound = 0.98)
free_calibration_parameter!(cal, ["NegativeElectrode", "ConductiveAdditive", "MassFraction"]; lower_bound = 0.005, upper_bound = 0.10)
...
free_calibration_parameter!(cal, ["NegativeElectrode", "Coating", "EffectiveDensity"]; lower_bound = 1400.0, upper_bound = 2600.0)
...
lines!(ax, base_output.time_series["CumulativeCapacity"], base_output.time_series["Voltage"], label = "Base")
lines!(ax, opt_output.time_series["CumulativeCapacity"], opt_output.time_series["Voltage"], label = "Calibrated")
scatter!(ax, exp_capacity, exp_voltage, label = "Experiment", markersize = 8)
