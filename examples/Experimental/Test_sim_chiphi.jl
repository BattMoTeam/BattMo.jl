...
nz_fac = 1#2
fac = 1#3*2    
H_mother, cellmap, facemap, nodemap, paramsz = basic_grid_example_p4d2(;
    nx=fac, ny=fac*2, nz=10*nz_fac, tab_cell_nx=3, tab_cell_ny=3
);
...
if include_cc
    ...
    cells = g.boundary_faces.neighbors[faces]
    boundary = Dict(
        "NegativeCurrentCollector" => Dict("cells" => cells, "boundaryfaces" => faces)
    )
    if do_plot
...
if !use_p2d
    init.object["PositiveElectrode"]["Coating"]["ActiveMaterial"]["InterDiffusionCoefficient"] =
        0
    init.object["NegativeElectrode"]["Coating"]["ActiveMaterial"]["InterDiffusionCoefficient"] =
        0
end
if !include_cc
...
if false
    states, cellSpecifications, reports, extra = run_battery(
        init; use_p2d=use_p2d, info_level=0, extra_timing=false
    );
else
    ##
    ...
    model = BattMo.setup_battery_model(
        init; use_groups=false, use_p2d=use_p2d, context=context
    )
    ...
    if true
        ...
        if false
            ...
            push!(
                varpreconds,
                BattMo.VariablePrecond(
                    Jutul.AMGPreconditioner(:ruge_stuben),
                    :Phi,
                    :charge_conservation,
                    nothing,
                ),
            )
            g_varprecond = BattMo.VariablePrecond(
                Jutul.ILUZeroPreconditioner(), :Global, :Global, nothing
            )
        else
            prec_org_s = Jutul.AMGPreconditioner(:ruge_stuben)
            ksolver_s = GenericKrylov(
            ...
            #push!(varpreconds,BattMo.VariablePrecond(s_prec,:C,:mass_conservation, nothing))
            g_varprecond = BattMo.VariablePrecond(
                Jutul.TrivialPreconditioner(), :Global, :Global, nothing
            )
    ...
    state0[:Control][:ControllerCV] = BattMo.SimpleControllerCV{Float64}(
        0.0, 0.0, false, discharging
    )
    cfg[:failure_cuts_timestep] = false
...
for ind in 1:3
    #ind = 1
    axlinesall = Axis(
        flines[1, ind]; yautolimitmargin=(0.1, 0.1), xautolimitmargin=(0.1, 0.1)
    )
    axlinesall_c = Axis(
        flines_c[1, ind]; yautolimitmargin=(0.1, 0.1), xautolimitmargin=(0.1, 0.1)
    )
    name = names[ind]
    sym = syms[ind]
