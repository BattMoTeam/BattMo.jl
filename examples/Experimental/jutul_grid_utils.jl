...
function basic_grid_example_p4d2(;
    nx=1, ny=1, nz=1, tab_cell_nx=0, tab_cell_ny=0, test=false
)
    ...
    add_cells_pcc = cat(
        getindex.(
            list_of_iterators[(end - pe_cc_nz + 1):end], [index_of_pcc .- x4_cells]
        )...;
        dims=1,
    )
    ...
    list_of_iterators2 = [
        (Nx * Ny * (i - 1) + 1):(Nx * (Ny * (i - 1) + ne_cc_ny)) for i in 1:Nz
    ]
    cells2 = cat(list_of_iterators2...; dims=1)

    index_of_ncc = cat(
        [(Nx * (i - 1) + 1):(Nx * (i - 1) + ne_cc_nx) for i in 1:ne_cc_ny]...; dims=1
    )

    if same_side
        add_cells_ncc = cat(
            getindex.(list_of_iterators[1:ne_cc_nz], [index_of_ncc .+ x0_cells])...; dims=1
        )
        setdiff!(cells1, add_cells_ncc)
    else
        add_cells_ncc = cat(
            getindex.(list_of_iterators2[1:ne_cc_nz], [index_of_ncc .+ x0_cells])...; dims=1
        )
