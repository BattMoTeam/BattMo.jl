...
if abs(h_correction_factor - 1.0) > 1.0e-12
...
function print_vector_diff_stats(label, a, b)
    d = a .- b
    return println("  $label diff: mean=$(mean(d)), min=$(minimum(d)), max=$(maximum(d)), rmse=$(sqrt(mean(d .^ 2)))")
end
...
function print_dT_stats(label, tj)
    ...
    return println("  $label: mean = $(mean(dT_loc)), min = $(minimum(dT_loc)), max = $(maximum(dT_loc))")
end
...
push!(
    iso_results, run_and_print_isolation_case(
...
if abs(base_cfg.h_correction_factor_case - 1.0) > 1.0e-12
