...
states, reports, extra = run_battery(
    init; use_p2d=true, info_level=1, max_step=nothing, general_ad=true
);
...
dG = solve_adjoint_sensitivities(
    model, states, reports, G; forces=forces, state0=state0, parameters=prm
)
...
opt_setup = setup_parameter_optimization(
    model, state0, prm, timesteps, forces, G, cfg; config=sim_cfg
);
...
f! = opt_setup.F!
g! = opt_setup.dF!
results, final_x = lb.lbfgsb(
    f!, g!, x0; lb=lower, ub=upper, iprint=prt, maxfun=200, maxiter=100
)
...
states_t, rep_t = simulate(
    state0, model, timesteps; parameters=prm_tuned, forces=forces, config=sim_cfg
);
