...
if (!simple)
    ...
    fn = string(
        dirname(pathof(BattMo)), "/../test/data/jsonfiles/old/3d_demo_geometry.json"
    )
...
if (do_plot)
    ...
    setups = ((:PeCc, :PeAm, "positive"), (:NeCc, :NeAm, "negative"))
    ...
    setups = ((:PeAm, "positive"), (:NeAm, "negative"))

    for setup in setups
        f3D = Figure(; size=(600, 650))
        ax3d = Axis3(f3D[1, 1]; title="Surface concentration in $(setup[2]) electrode")
        ...
        g = model[component].domain.representation
        Jutul.plot_cell_data!(
            ax3d, g, cs .- mincs; colormap=:viridis, colorrange=colorrange
        )
    ...
    setups = ((:C, "concentration"), (:Phi, "potential"))

    for setup in setups
        f3D = Figure(; size=(600, 650))
        ax3d = Axis3(f3D[1, 1]; title="$(setup[2]) in electrolyte")
        ...
        Jutul.plot_cell_data!(
            ax3d, g, val .- minval; colormap=:viridis, colorrange=colorrange
        )
