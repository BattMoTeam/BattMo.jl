...
![High-level architecture of some core workflows in BattMo.jl. The black arrows represent a simulation workflow and the striped arrows an optimization workflow. The gray arrows represent actions that are option for the user. The orange rounded blocks represent classes (structs) and the file blocks represent the input parameter sets and settings. \label{fig:backend}](./assets/battmo_backend.png "High-level Architecture")
...
The contour plots illustrate lithium concentrations and potentials, offering insights into the cell’s kinetic and transport processes. For example, the lithium concentration in the electrolyte shows expected values between $800$ and $1400 \unit{mol.m^{-3}}$. However, the order reduction in P2D models can overlook important geometric effects. P4D models address this limitation by accurately capturing these geometry-dependent phenomena. We can see this in the following example. 
