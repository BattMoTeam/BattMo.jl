...
The framework separates the workflow of running a simulation into 1) cell parameters and cycling protocols, which have direct correspondance to measurable and controllable quantities in experiments, 2) the definition of the physical model, which outlines the mathematical equations to solve, and 3) settings, which control the behaviour of the physical models, the simulation and the solver. The framework is represented in figure \ref{fig:backend} which shows the high-level architecture of running a simulation and an optimization. The workflows are configured through five different input sets, all of which can be provided in JSON format:

- Cell parameters: quantites describing the cell geometry, electrodes, and electrolyte materials.
- Cycling protocol: instructions of operating conditions, such as current profiles or voltage limits.
...
Each workflow starts by configuring a battery model representing the set of governing equations and constitutive laws to be solved in the simulation. A base model is instantiated (e.g., LithiumIonBattery) along with settings to configure it. For  insertion-based cells, the base model builds from the Doyle–Fuller–Newman (DFN) model [@Doyle1993ModelingCell], which can be extended with additional physics modules (e.g., SEI growth) depending on the configuration of the model settings.
...
A simulation is instantiated using the configured model, cell parameters, cycling protocols and optional simulation settings. At this stage, the input parameters are mapped to the model equations. Such parameters are used to setup, for example, the active material models, electrolyte properties, cell geometry and grids, load conditions, initial states, and timestepping. The simulation instance represents the complete system to be solved.

The simulation is solved by passing the simulation instance to the `solve` function, together with optional solver settings configuring the numerical solver. 

BattMo.jl’s solver leverages automatic differentiation and supports adjoint computations, enabling efficient calculation of parameter sensitivities and gradients of arbitrary objective functions. This enables gradient-based optimization workflows. In addition to the simulation workflow, figure \ref{fig:backend} also outlines the architecture of an optimization workflow. One example optimization application is parameter calibration from a voltage curve (VoltageCalibration). A parameter calibration fits some parameter values to experimental data. 

A calibration is setup via a VoltageCalibration instance, which accepts a simulation instance and the experimental data to fit. The VoltageCalibration instance is configured to either freeze or free certain parameters during the calibration and to set the bounds for each of the free parameters. A calibration is executed similarly as a simulation, i.e. by passing the calibration instance to the solve function. Calibration uses an adjoint-enabled solver to iteratively adjust the free parameters until the simulated voltage matches the experimental measurements within a tolerance.

The architecture is designed to enhance the user experience and provide a gentler learning curve by organizing core workflows and input parameters into well-defined steps and categories. By making the settings optional, users can initially focus on creating the virtual representation of the cell and defining cycling protocols. Later, they can adjust the settings to personalize configurations as they become more familiar with the software.
