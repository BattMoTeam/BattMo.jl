...
@testset "thermal" begin
    ...
    @test begin
        ...
        @test thermal_states[1][:Temperature][2] ≈ 298.14999996156644 atol = 1.0e-1
        @test thermal_states[1][:Temperature][100] ≈ 295.20098397016955 atol = 1.0e-1
        @test thermal_states[1][:Temperature][end] ≈ 298.1499999653794 atol = 1.0e-1

        @test thermal_states[40][:Temperature][2] ≈ 298.15000000713286 atol = 1.0e-1
        @test thermal_states[40][:Temperature][100] ≈ 288.3449387291137 atol = 1.0e-1
        @test thermal_states[40][:Temperature][end] ≈ 298.1500000109446 atol = 1.0e-1

        @test thermal_states[end][:Temperature][2] ≈ 298.150000007133 atol = 1.0e-1
        @test thermal_states[end][:Temperature][100] ≈ 290.68599259489594 atol = 1.0e-1
        @test thermal_states[end][:Temperature][end] ≈ 298.1500000109444 atol = 1.0e-1
