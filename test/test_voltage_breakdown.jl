...
@testset "voltage breakdown output" begin
	...
	@test begin
		...
		@test maximum(abs.(breakdown["OpenCircuitVoltage"] .- breakdown["OpenCircuitVoltageAverage"])) < 1e-12
		@test maximum(abs.(breakdown["OpenCircuitVoltageSurface"] .- (breakdown["OpenCircuitVoltageAverage"] .+ breakdown["PositiveSolidConcentrationOverpotential"] .+ breakdown["NegativeSolidConcentrationOverpotential"]))) < 1e-6
		@test maximum(abs.(breakdown["ElectrolytePotentialDrop"] .- (breakdown["ElectrolyteOhmicPotentialDrop"] .+ breakdown["ElectrolyteConcentrationOverpotential"]))) < 1e-6

		zero_current = findall(abs.(breakdown["Current"]) .< 1e-12)
		if !isempty(zero_current)
			@test maximum(abs.(breakdown["PositiveReactionOverpotential"][zero_current])) < 1e-6
			@test maximum(abs.(breakdown["NegativeReactionOverpotential"][zero_current])) < 1e-6
		end
