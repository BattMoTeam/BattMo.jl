...
function runP2DBatt(json_file)
    ...
    cfg = setup_config(cfg, model, timesteps, fraction_tot, dt_tot, i)
    ...
    negative_electrode_concentration = Array([
        [state[:NeAm][:Cs] for state in states] / 1000
    ])
    electrolyte_concentration = [state[:Elyte][:C] for state in states] / 1000
    positive_electrode_concentration = Array(
        [[state[:PeAm][:Cs] for state in states]] / 1000
    )
...
function setup_config(cfg, model::MultiModel, timesteps, fraction_tot, dt_tot, i)
    if model[:Control].system.policy isa CyclingCVPolicy
        cfg[:tolerances][:global_convergence_check_function] =
            (model, storage) -> BattMo.check_constraints(model, storage)
...
json_file = string(
    dirname(pathof(BattMo)), "/../test/data/jsonfiles/old/p2d_40_jl_chen2020.json"
)
